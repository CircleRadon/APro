...
class Box2MaskHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        self.zeta_gl = zeta_gl
        self.zeta_gh = zeta_gh
        self.mst = MinimumSpanningTree(Global_APro.norm2_distance)
        self.global_apro = Global_APro()
        self.local_apro = Local_APro(kernel_size=kernel_size, zeta_s=zeta_s)
    ...
    def loss(self, all_cls_scores, all_mask_preds,  gt_labels_list,
        ...
        losses_cls, loss_project, loss_global, loss_local = multi_apply(
        ...
        loss_dict['loss_global'] = loss_global[-1]
        loss_dict['loss_local'] = loss_local[-1]

        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_project_i, loss_global_i,  loss_local_i in zip(losses_cls[:-1], loss_project[:-1], loss_global[:-1], loss_local[:-1]):
            loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
            loss_dict[f'd{num_dec_layer}.loss_project'] = loss_project_i
            loss_dict[f'd{num_dec_layer}.loss_global'] = loss_global_i
            loss_dict[f'd{num_dec_layer}.loss_local'] = loss_local_i

            num_dec_layer += 1
        return loss_dict


    def loss_single(self, cls_scores, mask_preds, gt_labels_list, gt_masks_list, img_metas, norm_img):
        ...
        if mask_targets.shape[0] == 0:
            ...
            loss_global_term = mask_preds.sum()
            loss_local_term = mask_preds.sum()

            return loss_cls, loss_project, loss_global_term, loss_local_term
        ...
        local_soft_pseudo = self.local_apro(img_targets, mask_preds.squeeze(dim=1), box_mask_targets.squeeze(dim=1))
        loss_local_term = torch.abs(mask_preds - local_soft_pseudo.unsqueeze(dim=1)) * box_mask_targets
        box_regions = box_mask_targets.sum()
        loss_local = loss_local_term.sum() / box_regions

        # global affinity propagation
        img_targets = _scale_target(img_targets)
        mask_preds = _scale_target(mask_preds)

        global_soft_pseudo = self.global_apro(mask_preds, img_targets, target_img_tree, zeta_g=self.zeta_gl)
        global_soft_pseudo = self.global_apro(global_soft_pseudo, global_soft_pseudo, target_img_tree, zeta_g=self.zeta_gh)
        global_soft_pseudo = F.interpolate(global_soft_pseudo, pred_shape, mode='bilinear', align_corners=False)
        mask_preds_scaled = F.interpolate(mask_preds, pred_shape, mode='bilinear', align_corners=False)

        loss_global_term = torch.abs(global_soft_pseudo - mask_preds_scaled) * box_mask_targets
        loss_global = loss_global_term.sum() / box_regions

        return loss_cls, loss_project, loss_global, loss_local
