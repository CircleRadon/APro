...
class BoxSOLOv2Head(BaseModule):
    ...
    def loss(self,
        ...
        ins_label_list, cate_label_list, ins_ind_label_list, scale_img_list = multi_apply(
        ...
        for ins_pred, box_mask, img_target in zip(ins_preds, ins_labels, img_target):
            ...
            global_soft_pseudo = self.global_apro(mask_pred, img_target, img_mst_tree, zeta_g=self.zeta_gl)
            global_soft_pseudo = self.global_apro(global_soft_pseudo, global_soft_pseudo, img_mst_tree, zeta_g=self.zeta_gh)
            loss_global_term = torch.abs(global_soft_pseudo - mask_pred) * box_mask_target
            box_regions = box_mask_target.sum((1, 2, 3)).clamp(min=1)
            loss_global_term = loss_global_term.sum((1, 2, 3)) / box_regions
            loss_global.append(2.0 * loss_global_term)


            # local affinity propagation
            local_soft_pseudo = self.local_apro(img_target, mask_pred.squeeze(dim=1), box_mask_target.squeeze(dim=1))
            loss_local_term = torch.abs(mask_pred - local_soft_pseudo.unsqueeze(dim=1)) * box_mask_target
            loss_local_term = loss_local_term.sum((1, 2, 3)) / box_regions
            loss_local.append(loss_local_term)

        loss_project = torch.cat(loss_project).mean()
        loss_global = torch.cat(loss_global).mean()
        loss_local = torch.cat(loss_local).mean()
    ...
    def solo_target_single(self,
        ...
        for (lower_bound, upper_bound), stride, featmap_size, num_grid \
            ...
            cate_label_list.append(cate_label)
            ins_ind_label_list.append(ins_ind_label)
        return ins_label_list, cate_label_list, ins_ind_label_list, scale_img_list
