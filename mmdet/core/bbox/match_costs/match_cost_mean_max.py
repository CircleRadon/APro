...
class BoxMatchingCost:
    ...
    def __call__(self, mask_preds, gt_box_masks):
        ...
        dice_cost_proj = dice_cost_x + dice_cost_y


        mask_pred_mean_x = mask_preds.mean(dim=3, keepdim=True).max(dim=2, keepdim=True)[0]
        gt_mask_mean_x = gt_box_masks.float().mean(dim=3, keepdim=True).max(dim=2, keepdim=True)[0]

        mask_pred_mean_y = mask_preds.mean(dim=2, keepdim=True).max(dim=3, keepdim=True)[0]
        gt_mask_mean_y = gt_box_masks.float().mean(dim=2, keepdim=True).max(dim=3, keepdim=True)[0]

        # import pdb
        # pdb.set_trace()
        dice_cost_mean_x = self.bin_dice_loss(mask_pred_mean_x, gt_mask_mean_x)
        dice_cost_mean_y = self.bin_dice_loss(mask_pred_mean_y, gt_mask_mean_y)

        # import pdb
        # pdb.set_trace()

        dice_cost_mean = dice_cost_mean_x + dice_cost_mean_y
        ...
        # return (0.75 * dice_cost_proj + 0.25 * dice_cost_mean) * self.weight
        # print("dice_cost_proj", dice_cost_proj)
        # print("dice_cost_mean", dice_cost_mean)
        return (dice_cost_proj + dice_cost_mean) * self.weight